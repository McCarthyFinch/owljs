...
importPackage(Packages.uk.ac.manchester.cs.jfact);
importPackage(Packages.uk.ac.manchester.cs.factplusplus.owlapiv3);
importPackage(Packages.com.clarkparsia.owlapi.explanation.util);
importPackage(Packages.com.clarkparsia.owlapi.explanation.io);
importPackage(Packages.com.clarkparsia.owlapi.explanation);

// importPackage(Packages.com.clarkparsia.owlapi.explanation.io.manchester);

importPackage(Packages.owltools.reasoner);
...
importPackage(Packages.org.semanticweb.owlapi.debugging);

//explanation support
importPackage(Packages.org.semanticweb.owl.explanation.api);
...
if (java.lang.System.getenv().containsKey("OWLTOOLS_JAR_PATH")) {
    ...
    var modulePath = java.lang.System.getenv().get("RINGO_MODULE_PATH");
    addToClasspath(modulePath+"/owljs/jars/FaCTpp-OWLAPI-3.4-v1.6.3.jar");
    addToClasspath(modulePath+"/owljs/jars/owlexplanation-1.1.0.jar");
    addToClasspath(modulePath+"/owljs/jars/telemetry-1.0.0.jar");
}
...
OWL.prototype.createOntology = function(iriOrOntID, imports, anns, versionIRI) {
    ...
    if (imports != null) {
        for (var k in imports) {
            var imp = imports[k];
            if (imp.getOntologyID != null) {
                imp = imp.getOntologyID().getOntologyIRI();
            }
            var ai = new AddImport(this.ontology, this.getOWLDataFactory().getOWLImportsDeclaration(this.getIRI(imp)));
            var changes = manager.applyChange(ai);
        }
        this.log("|ImportsClosure| = " + this.ontology.getImportsClosure().size());
        this.log("|DirectImportsDocuments| = " + this.ontology.getDirectImportsDocuments().size());
        this.log("|DirectImports| = " + this.ontology.getDirectImports().size());
    }
    if (anns != null) {
        for (var k in anns) {
            var ann = anns[k];
 ...
 * 
 * Loads an ontology from an IRI. The current ontology
 * is set to be this ontology (unless addToImports is set)
 * 
 * Arguments:
 ...
 *
 * Opts:
 *  - addToImports: boolean. If true, then the loaded ontology will be added as an import of the current ontology, if present
 *
 *
 * See Also:
 *  - <loadFile>
 */
OWL.prototype.loadOntology = function(iri, opts) {
    if (iri == null) {
    ...
    this.log("Loading from "+iri);
    var newOntology = manager.loadOntologyFromOntologyDocument(iri);
    if (opts != null && opts.addToImport) {
        this.log("Adding import to "+iri+" from "+this.getOntology());
        var change = new AddImport(this.getOntology(), 
                                   this.getOWLDataFactory().getOWLImportsDeclaration(newOntology.getOntologyID().getOntologyIRI()));
 ...
 * 
 * Arguments:
 *  - file : path to a catalog XML file (OPTIONAL; defaults to "catalog-v001.xml")
 */
OWL.prototype.addCatalog = function(file) {
    if (file == null) {
        file = "catalog-v001.xml";
    }
    if (!require('fs').exists(file)) {
        this.warn("No catalog found");
        return;
    }
 ...
 *  <owlapi.OWLClass> []
 */
OWL.prototype.getInferredSubClasses = function(cls, isDirect, isReflexive, isRemoveFake) {
    if (isDirect == null) {
        isDirect = true;
    }
    ...
    // experimental - see notes on materialization below
    if (isRemoveFake && this.fakeClassMap != null) {
        var fcm = this.fakeClassMap;
        return rl.filter(function(x) { return !fcm[x.toString() + ""] } );
    }
    else {
        return rl;
    }
 ...
 *  - supercls : an <owlapi.OWLClassExpression>
 *
 * Returns:
 *  true is subcls is a subclass of supercls
 */
OWL.prototype.isInferredSubClassOf = function(subcls, cls) {
    var subs = this.getInferredSubClasses(cls, false, true);
    for (var k in subs) {
        if (subs[k].equals(subcls)) {
            return true;
        }
    }
    return false;
...
/* Function: getInferredSuperProperties
 *
 * Uses an <owlapi.OWLReasoner> to find inferred superproperties for a property
 *
 * Arguments:
 ...
 * Returns:
 * 
 *  list of <owlapi.OWLObjectProperty> or  <owlapi.OWLDataProperty>
 */
OWL.prototype.getInferredSuperProperties = function(prop, isDirect, isReflexive) {
    if (isDirect == null) {
        isDirect = true;
    }
    var jl;
    if (prop instanceof OWLObjectPropertyExpression) {
        jl = this.getReasoner().getSuperObjectProperties(prop, isDirect).getFlattened();
    }
    else {
        jl = this.getReasoner().getSuperDataProperties(prop, isDirect).getFlattened();
    }
    var rl = this.a2l(jl.toArray());
    if (isReflexive) {
        //rl = rl.concat(this.getInferredEquivalentProperties(cls)); TODO
    }
    return rl;
}
 ...
 *  - isDirect : boolean. default is false
 *
 *
 * Returns:
 *  <owlapi.OWLClassExpression> []
 */
 ...
 * 
 * Side effects:
 * - pollutes ontology with an additional import declaration, containing materialized classes;
 * - todo: find a clean way of separating these, as they will show up in <getInferredSuperClasses> calls
 * 
 * Arguments:
 *  - cls : an OWLClass
 ...
 */
OWL.prototype.getAncestorsOver = function(cls, prop, isDirect, isReflexive) {
    var mat = this.materializeExistentialExpressions(prop, isReflexive);
    var supers = this.getInferredSuperClasses(cls, false, false);
    var lookup = mat.lookup;
    var ancs = [];
    var redundant = [];
    var this2 = this;
   // this.log("Finding redundant...");
    supers.forEach(function(x) {
        ...
        if (orig != null) {
            ancs.push(orig);
            if (isDirect) {
                var supers2 = this2.getInferredSuperClasses(orig, false, false);
                supers2.forEach(function(x2) {
                    var k2 = x2.toString() + "";
                    redundant.push(k2);
                    var orig2 = lookup[k2];
                    if (orig2 != null) {
                        redundant.push(orig2.toString() + "");
                    }
                });
            }
        }
    });
    //this.log("DONE Finding redundant...");

    ancs = ancs.filter( function(x) { return redundant.indexOf( x.toString() + "" ) == -1 } );
...
OWL.prototype.materializeExistentialExpressions = function(prop, isReflexive) {
    ...
    if (this.matCache[suffix] != null) {
        return this.matCache[suffix];
    }
    this.log("Materializing "+prop+" suffix="+suffix);
    if (this.fakeClassMap == null) {
        this.fakeClassMap = {};
    }
    this.log("Materializing existential expressions for "+prop);
    ...
    for (var k in cl) {
        ...
        this.fakeClassMap[tmpCls.toString()  + ""] = true;
    ...
    // todo - allow removal
    this.log("Caching "+suffix+" Entries="+n);
    this.matCache[suffix] = mat;
    return mat;
}

OWL.prototype.getEquivalentClasses = function(obj) {
    return javautil.collectionToJsArray(obj.getEquivalentClasses(this.getOntology().getImportsClosure()));
}

OWL.prototype.getPropertyValues = function(obj) {
    var pvm = obj.getObjectPropertyValues(this.getOntology());
    var props = javautil.collectionToJsArray(pvm.keySet());
    var pvs = [];
    for (var k in props) {
        var p = props[k];
        var vals = javautil.collectionToJsArray(pvm.get(p));
        vals.forEach(
            function(v) {
                pvs.push({
                    property : p,
 ...
 * <owlapi.OWLAxiom> []
 */
OWL.prototype.grepAxioms = function(grepFunc, isNegated, isReplace, incClosure) {
    var inAxioms = this.getAllAxioms(null, incClosure);
    if (isNegated == null) {
        isNegated = false;
    }
    var filteredAxioms = [];
 ...
 * <owlapi.OWLAxiom> []
 */
OWL.prototype.sedAxioms = function(sedFunc, incClosure) {
    var inAxioms = this.getAllAxioms(null, incClosure);
    var newAxioms = [];
    var rmAxioms = [];
    for (var k in inAxioms) {
        var ax = inAxioms[k];
        var ax2 = sedFunc.call(this, ax, this);
        if (ax2 == null) {
        }
...
OWL.prototype.ontologyToAxiomaticJSON = function() {
    return this.toAxiomaticJSON(this.getOntology());
}

OWL.prototype.toAxiomaticJSON = function(obj) {
    var ctx = this;
    ...
    if (obj instanceof OWLOntology) {
        var oid = obj.getOntologyID();
        var json =
            {
                iri : this.toAxiomaticJSON( oid.getOntologyIRI() ),
                annotations : javautil.collectionToJsArray(obj.getAnnotations()).map( function(ax) { return ctx.toAxiomaticJSON(ax) }),
                axioms : javautil.collectionToJsArray(obj.getAxioms()).map( function(ax) { return ctx.toAxiomaticJSON(ax) }),
            };
        if (oid.getVersionIRI() != null) {
            json.versionIRI = this.toAxiomaticJSON( oid.getVersionIRI() );
        }
        return json;
    }
    ...
    if (obj instanceof OWLAnnotation) {
        return {
            property : this.toAxiomaticJSON(obj.getProperty()),
            value : this.toAxiomaticJSON(obj.getValue())
        }
    }
    if (obj instanceof OWLLiteral) {
        return obj.getLiteral().toString() + ""; // TODO
    }
    ...
    if (obj.getAxiomType != null) {
        // quack! I'm an axiom
        json.type = obj.getAxiomType() + "";

        if (obj instanceof OWLDeclarationAxiom) {
            json.args = [
                this.toAxiomaticJSON(obj.getEntity())
            ];
        }
        else if (obj instanceof OWLObjectPropertyCharacteristicAxiom) {
            json.args = [
                this.toAxiomaticJSON(obj.getProperty())
            ];
        }
        else if (obj.getSubClass != null) {
            // quack: subClassOf
            json.args = [
                this.toAxiomaticJSON(obj.getSubClass()),
                this.toAxiomaticJSON(obj.getSuperClass())
            ];
        } else if (obj.getIndividual != null) {
            // quack: classAssertion
            json.args = [
                this.toAxiomaticJSON(obj.getClassExpression()),
                this.toAxiomaticJSON(obj.getIndividual())
            ];
        } else if (obj.getSubProperty != null) {
            // quack: subPropertyOf
            json.args = [
                this.toAxiomaticJSON(obj.getSubProperty()),
                this.toAxiomaticJSON(obj.getSuperProperty())
            ];
        } else if (obj.getPropertyChain != null) {
            // quack: subPropertyChain
            json.args = 
                javautil.collectionToJsArray(obj.getPropertyChain()).map(function(x) { return ctx.toAxiomaticJSON(x) });
            json.args.push( this.toAxiomaticJSON(obj.getSuperProperty()) );
        } else if (obj.getFirstProperty != null) {
            // quack: inverseProperties
            json.args = [
                this.toAxiomaticJSON(obj.getFirstProperty()),
                this.toAxiomaticJSON(obj.getSecondProperty())
            ];
        } else if (obj.getProperties != null) {
            // quack: n-ary property axiom
            json.args = 
                javautil.collectionToJsArray(obj.getProperties()).map(function(x) { return ctx.toAxiomaticJSON(x) });
        } else if (obj.getClassExpressions != null) {
            // quack: n-ary
            json.args = 
                javautil.collectionToJsArray(obj.getClassExpressions()).map(function(x) { return ctx.toAxiomaticJSON(x) });
        }
        else if (obj.getSubject != null) {
            // quack: annotationAssertion
            json.args = [
                this.toAxiomaticJSON(obj.getProperty()),
                this.toAxiomaticJSON(obj.getSubject()),
                this.toAxiomaticJSON(obj.getValue())
            ];
        }
        else if (obj.getRange != null) {
            // quack: propertyRange
            json.args = [
                this.toAxiomaticJSON(obj.getProperty()),
                this.toAxiomaticJSON(obj.getRange())
            ];
        }
        else if (obj.getDomain != null) {
            // quack: propertyDomain
            json.args = [
                this.toAxiomaticJSON(obj.getProperty()),
                this.toAxiomaticJSON(obj.getDomain())
            ];
        }
        else {
            console.warn("Unhandled: " + json.type + " obj="+obj);
        }

        var anns = javautil.collectionToJsArray(obj.getAnnotations());
        if (anns.length > 0) {
            json.annotations = anns.map( function(ann) { return ctx.toAxiomaticJSON(ann) } );
        }

    }
    else if (obj.getClassExpressionType != null) {
        // quack! I'm an expression
        json.type = obj.getClassExpressionType() + "";
        if (obj.getFiller != null) {
            json.args = [
                this.toAxiomaticJSON(obj.getProperty()),
                this.toAxiomaticJSON(obj.getFiller())
            ];
        }
        else if (obj.getOperands != null) {
            // quack: n-ary
            json.args = 
                javautil.collectionToJsArray(obj.getOperands()).map(function(x) { return ctx.toAxiomaticJSON(x) });
        }
        else {
            console.warn("Unhandled: " + json.type + " obj="+obj);
        }
...
OWL.prototype.add = function(ax) {
    ...
    else if (ax instanceof OWLOntologyChange) {
        return this.applyChange(ax);
    }
    else if (ax.concat != null) {
        return this.addAxioms(ax);
    }
    else {
        this.warn("DID NOT ADD: "+ax);
    }
}
...
OWL.prototype.getAnnotationAssertions = function(obj,prop) {
    if (!(obj instanceof OWLNamedObject)) {
        // note: it doesn't matter what kind of OWLNamedObject we create here
        if (!(obj instanceof IRI)) {
            obj = IRI.create(obj);
    ...
    for (var k in onts) {
        var ont = onts[k];
        var newAxs = javautil.collectionToJsArray(ont.getAnnotationAssertionAxioms(obj.getIRI()));
        if (prop != null) {
            axs = axs.concat( newAxs.filter( function(ax) { return ax.getProperty().equals(prop) } ) );
        }
        else {
            axs = axs.concat( newAxs );
        }
    }
    return axs;
}
...
OWL.prototype.getIRI = function(iriStr) {
    if (iriStr == null) {
        console.warn("Passed null to getIRI");
    }
...
OWL.prototype.keyMatches = function(key, obj) {
    ...
    // key is a string - match against IRIs, labels, then fragments
    var iri = this.getIRI(obj);

    if (iri.toString() + "" == key) {
        return true;
    }
    if (iri.getFragment().toString() == key) {
        return true;
    }
 ...
 *
 * Arguments:
 *  incClosure : if true, uses entire ontology imports closure
 *
 * Returns:
 * <owlapi.OWLClass> []
 */
OWL.prototype.getClasses = function(incClosure) {
    if (incClosure == null) {
        incClosure = true;
    }
    return this.a2l(this.getOntology().getClassesInSignature(incClosure).toArray());
}
 ...
 */
OWL.prototype.getIndividuals = function(incClosure) {
    if (incClosure == null) {
        incClosure = true;
    }
    return this.a2l(this.getOntology().getIndividualsInSignature(incClosure).toArray());
}
 ...
 */
OWL.prototype.getObjectProperties = function(incClosure) {
    if (incClosure == null) {
        incClosure = true;
    }
    return this.a2l(this.getOntology().getObjectPropertiesInSignature(incClosure).toArray());
}
 ...
 */
OWL.prototype.getAnnotationProperties = function(incClosure) {
    if (incClosure) {
        var onts = this.getOntologyImportClosure();
        var aps = new HashSet();
        for (var k in onts) {
            aps.addAll(onts[k].getAnnotationPropertiesInSignature());
        }
        return javautil.collectionToJsArray(aps);
    }
    else {
 ...
 * <owlapi.OWLObjects> []
 */
OWL.prototype.getAllObjects = function(incClosure) {
    if (incClosure == null) {
        incClosure = true;
    }
    var objs = [];
    var classes = this.getClasses(incClosure);
    objs = objs.concat(classes);

    var rels = this.getObjectProperties(incClosure);
    objs = objs.concat(rels);

    var aps = this.getAnnotationProperties(incClosure);
    objs = objs.concat(aps);

    var inds = this.getIndividuals(incClosure);
    objs = objs.concat(inds);
...
/* Function: getClassesWithAxioms
 *
 * Returns any classes in ontology that have at least one axiom
 * (logical or non-logical)
 *
 * Arguments:
 *  incClosure: boolean; see <getClasses>
 *
 * Rationale:
 *
 ...
 *
 * Returns:
 *  a list of <owlapi.OWLClass>
 */
OWL.prototype.getClassesWithAxioms = function(incClosure) {
    var x = this;
    return this.getClasses(incClosure).filter(function(c) { return x.getAllAxioms(c, incClosure).length > 0 } );
}

//OWL.prototype.getDeclaredClasses = function(incClosure) {
//    if (incClosure == null) {
//        incClosure = true;
//    }
//    var dSet = this.getOntology().getAxioms(AxiomType.DECLARATION, incClosure);
//    var cSet = this.this.getOntology().getClassesInSignature(incClosure);
//    cSet.retainAll(d
//    var decs = this.a2l(.toArray());
//    var objs = decs.map(function(d)
//}

// deprecated? does not yield annotation assertions
OWL.prototype.getAxioms = function(obj) {
    console.warn("Deprecated: use getAllAxioms");
    //return this.a2l(this.getOntology().getAxioms(obj).toArray());
    return this.getAllAxioms(obj);
}
 ...
 *
 * Arguments:
 *  - obj : an <owlapi.OWLObject> (optional)
 *  - incClosure: boolean (optional)
 *
 * Returns:
 * <owlapi.OWLAxiom> []
 */
OWL.prototype.getAllAxioms = function(obj, incClosure) {
    var axioms = new HashSet();
    var onts = [this.getOntology()];
    if (incClosure == null || incClosure) {
        onts = this.getOntologyImportClosure();
    }
    onts.forEach(
        function(ont) {
            if (obj == null) {
                ...
                axioms.addAll(ont.getAxioms(obj));
                
                if (obj.getIRI != null) {
                    var aas = ont.getAnnotationAssertionAxioms(obj.getIRI());
                    if (aas != null) {
                        axioms.addAll(aas);
                    }
                }
    ...
    return javautil.collectionToJsArray(axioms);
}


// experimental
OWL.prototype.getAxiomsReferencing = function(obj, incClosure) {
    var axioms = this.getAllAxioms(null, incClosure);
    axioms = axioms.filter(
        function(ax) {
            // TODO - only works for OPs so far
            if (ax.getObjectPropertiesInSignature != null) {
                return ax.getObjectPropertiesInSignature().contains(obj);
            }
        });
    return axioms;
}

OWL.prototype.getOntologyImportClosure = function() {
    return javautil.collectionToJsArray(this.getOntology().getImportsClosure());
}
...
/* Function: explanationNestedClassExpression
 *
 * Create an explanation for an inference in the ontology
 *
 * Arguments:
 *  - axiom : <owlapi.OWLAxiom>
 *
 *
 * Returns:
 * <HashSet> Nested Class Expression HashSet
 */
OWL.prototype.explanationNestedClassExpression = function(axiom) {
    var genFac = org.semanticweb.owl.explanation.api.ExplanationManager.createLaconicExplanationGeneratorFactory(this.getReasonerFactory());
    var gen = genFac.createExplanationGenerator(this.getOntology());

    // Get our explanations.  Ask for a maximum of 5.
    var expl = gen.getExplanations(axiom, 5);

    var iterator = expl.iterator();

    var smallestSize = null;
    var nce = null;
    while(iterator.hasNext()) {
        var exp = iterator.next();
        var size = exp.getSize();

        //get the simplest explanation
        if (smallestSize === null || size < smallestSize) {
            nce = exp.getNestedClassExpressions();
            smallestSize = size;
        }
    }
...
/* Function: max
 *
 * Creates a maximum cardinality restriction (C P SOME C) using a factory
 *
 * Arguments:
 *  - cardinality: <integer>
 *  - p : <owlapi.OWLProperty>
 ...
 *
 * Returns:
 * <owlapi.OWLObjectMaxCardinality> or <owlapi.OWLDataMaxCardinality>
 */
OWL.prototype.max = function(cardinality, p, filler) {
    if (p instanceof OWLDataPropertyExpression) {
        return this.getOWLDataFactory().getOWLDataMaxCardinality(cardinality, p, filler);
    }
    else {
        return this.getOWLDataFactory().getOWLObjectMaxCardinality(cardinality, p, filler);
    }
}
...
/* Function: min
 *
 * Creates a minimum cardinality restriction restriction (C P SOME C) using a factory
 *
 * Arguments:
 *  - cardinality: <integer>
 *  - p : <owlapi.OWLProperty>
 ...
 *
 * Returns:
 * <owlapi.OWLObjectMinCardinality> or <owlapi.OWLDataMinCardinality>
 */
OWL.prototype.min = function(cardinality, p, filler) {
    if (p instanceof OWLDataPropertyExpression) {
        return this.getOWLDataFactory().getOWLDataMinCardinality(cardinality, p, filler);
    }
    else {
        return this.getOWLDataFactory().getOWLObjectMinCardinality(cardinality, p, filler);
    }
}
...
/* Function: exact
 *
 * Creates an exact cardinality restriction (C P SOME C) using a factory
 *
 * Arguments:
 *  - cardinality: <integer>
 *  - p : <owlapi.OWLProperty>
 ...
 *
 * Returns:
 * <owlapi.OWLObjectExactCardinality> or <owlapi.OWLDataExactCardinality>
 */
OWL.prototype.exact = function(cardinality, p, filler) {
    if (p instanceof OWLDataPropertyExpression) {
        return this.getOWLDataFactory().getOWLDataExactCardinality(cardinality, p, filler);
    }
    else {
        return this.getOWLDataFactory().getOWLObjectExactCardinality(cardinality, p, filler);
    }
}


/* Function: allValuesFrom
 *
 * Creates an universal restriction (P ALL C) using a factory
 *
 * Arguments:
 *  - p : <owlapi.OWLProperty>
 *  - filler : <owlapi.OWLClassExpression>
 ...
 *
 * Returns:
 * <owlapi.OWLObjectAllValuesFrom> or <owlapi.OWLDataAllValuesFrom>
 */
OWL.prototype.allValuesFrom = function(p, filler) {
    if (p instanceof OWLDataPropertyExpression) {
        return this.getOWLDataFactory().getOWLDataAllValuesFrom(p, filler);
    }
    else {
        return this.getOWLDataFactory().getOWLObjectAllValuesFrom(p, filler);
    }
}
...
/* Function: hasValue
 *
 * Creates a has-value restriction (P VALUE FILLER) using a factory
 *
 * Arguments:
 *  - p : <owlapi.OWLProperty>
 *  - filler : <owlapi.OWLIndividual>
 *
 *
 * Returns:
 * <owlapi.OWLObjectHasValue> or <owlapi.OWLDataHasValue>
 */
OWL.prototype.hasValue = function(p, filler) {
    if (p instanceof OWLDataPropertyExpression) {
        return this.getOWLDataFactory().getOWLDataHasValue(p, filler);
    }
    else {
        return this.getOWLDataFactory().getOWLObjectHasValue(p, filler);
    }
}
...
OWL.prototype.unionOf = function() {
    var xset = new java.util.HashSet();
    var isData = false;
    var a = argsToArray.apply(this,arguments);
    for (var k=0; k<a.length; k++) {
        // todo - detect isData
        xset.add(a[k]);
    }
...
OWL.prototype.inverseOf = function(p) {
    return this.getOWLDataFactory().getOWLObjectInverseOf(p);
}
...
OWL.prototype.subPropertyOf = function(a,b) {
    // TODO - obj vs data vs ann
    return this.getOWLDataFactory().getOWLSubObjectPropertyOfAxiom(a,b);
}

OWL.prototype.subPropertyChainOf = function(chain,b) {
    var cx = this;
    chain = 
        chain.map(function(p){
            if (p.isInverseFunctional != null) {
                return p; // QUACK
            }
            else {
                return cx.objectProperty(p)
            }
        });
    return this.getOWLDataFactory().getOWLSubPropertyChainOfAxiom(chain,b);
}
...
OWL.prototype.genIRI = function() {
    ...
    var iriStr = "http://purl.obolibrary.org/obo/"+this.config.idspace+"_"+localId; // TODO - make configurable
...
OWL.prototype.log = function(msg) {
    if (this.config != null && this.config.logLevel > 0) {
        console.log(msg);
    }
}
...
/* Namespace: owlapi
 *
 * owl.js makes use of the OWLAPI; minimal documentation on select
 * classes are provided here. For full documentation, consult the
 * owlapi documentation (http:://owlapi.sourceforge.net).
 *
 * About: OWLOntology
 *
